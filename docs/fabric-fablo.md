...
The default setup runs one channel (`my-channel1`) with two peers in `Org1` and the `assignment-name-placeholder` chaincode deployed as a chaincode-as-a-service (CCaaS).
...
| Channel | `my-channel1` |
| Chaincode | `assignment-name-placeholder` (CCaaS, Node.js) |
...
```console
# Store a value
$ ./scripts/cc.sh assignment-name-placeholder invoke put mykey hello

# Read it back
$ ./scripts/cc.sh assignment-name-placeholder query get mykey

# Same, but print the request and response
$ ./scripts/cc.sh -v assignment-name-placeholder query get mykey
```
